...
Процедура _ЗаписатьЖурналВРегистр(Замер, ТЗЖурнала, ДатаЗаписи=Неопределено, ПутьКЖурналу="") Экспорт
	НаборЗаписей = РегистрыСведений.Журнал.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Замер.Установить(Замер);
	НаборЗаписей.Отбор.ДатаЗаписи.Установить(Замер.ДатаПоследнейЗаписи);
...
Процедура ЗаписатьЖурналВРегистр(Замер, ТЗЖурнала, ДатаЗаписи=Неопределено, ПутьКЖурналу="") Экспорт
	...
	//ТЗДляЗаписи = ТЗЖурнала.СкопироватьКолонки();
	ТЗДляЗаписи = НаборЗаписей.Выгрузить();
	
	Для Каждого стрЖурнала Из ТЗЖурнала Цикл
		Если ТЗДляЗаписи.Найти(стрЖурнала.КлючЗаписи, "КлючЗаписи") = Неопределено Тогда
...
Процедура _ЗаписатьЖурналВРегистр2(Замер, ТЗЖурнала, ДатаЗаписи=Неопределено, ПутьКЖурналу="") Экспорт
	НаборЗаписей = РегистрыСведений.Журнал.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Замер.Установить(Замер);
	НаборЗаписей.Прочитать();
	
	ТЗНаборЗаписей = НаборЗаписей.Выгрузить(, "КлючЗаписи");
	НаборЗаписей = Неопределено;
	
	ТЗДляЗаписи = ТЗЖурнала.СкопироватьКолонки();
	//ТЗДляЗаписи = НаборЗаписей.ВыгрузитьКолонки();
	НаборЗаписей = Неопределено;
	
	Для Каждого стрЖурнала Из ТЗЖурнала Цикл
		Если ТЗНаборЗаписей.Найти(стрЖурнала.КлючЗаписи, "КлючЗаписи") = Неопределено Тогда
			НоваяЗапись = ТЗДляЗаписи.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяЗапись, стрЖурнала);
		КонецЕсли;
	КонецЦикла;
	...
	НаборЗаписей = РегистрыСведений.Журнал.СоздатьНаборЗаписей();
	//НаборЗаписей.Отбор.Замер.Установить(Замер);	
	НаборЗаписей.Загрузить(ТЗДляЗаписи);
