...
Процедура ПрочитатьЖурнал(ПутьКЖурналу, Замер=Неопределено) Экспорт
	...
	Для Каждого ФайлЖурнала Из МассивФайлов Цикл
		КлючЗаписи = РазобратьФайл(ФайлЖурнала, ТЗЖурнала);
	КонецЦикла;
	...
	Если Замер <> Неопределено Тогда
		...
		ЗамерОбъект.КлючПоследнегоСобытия = КлючЗаписи;
...
//  
// Массив из Строка
Функция РазобратьФайл(ФайлЖурнала, ТЗЖурнала)
	ШаблонСклейкиСтрок = "\d{2}:\d{2}.\d{6}-\d";
	ШаблонИмениФайла = "(\d{2})(\d{2})(\d{2})(\d{2})";
	
	ШаблонПарсинга1 = ",(\w+)='([^']+)";
	ШаблонПарсинга2 = ",(\w+)=""([^""]+)";
	ШаблонПарсинга3 = ",([A-Za-z0-9А-Яа-я:]+)=([^,]+)";
	...
	Для Каждого СтрокаТЖ Из МассивСтрокТЖ Цикл
		...
		Если РезультатПоиска.НачальнаяПозиция <> 0 Тогда
			...
			ДлительностьСек = СтруктураСтроки.Длительность/1000000;
			ДлительностьМилиСек = СтруктураСтроки.Длительность/1000;
			
			СтруктураСтроки.Длительность = Число(Длительность);
