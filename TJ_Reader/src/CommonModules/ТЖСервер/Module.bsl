...
//  ТЗЖурнала - ТаблицаЗначений - данные журнала 
//  Замер - СправочникСсылка.Замеры - Замер
//  Фильтры - Структура - Фильтры настроеные в форме журнала
//  
// Возвращаемое значение:
// 	Дата - Дата время 
Функция РазобратьФайл(ФайлЖурнала, ТЗЖурнала, Замер, Фильтры) Экспорт
	...
	Для Каждого СтрокаТЖ Из МассивСтрокТЖ Цикл
		ОтметкаВремени = Сред(СтрокаТЖ, 1, 12);
		...
		КлючЗаписи = ИмяФайла+ОтметкаВремени; 
		СтруктураСтроки.КлючЗаписи = КлючЗаписи;
		НоваяСтрока.КлючЗаписи = КлючЗаписи;
		НоваяСтрока.ОтметкаВремени = ОтметкаВремени;
...
Функция СтруктураСтроки()
	...
	Шаблоны.Вставить("ОтметкаВремени", "");
...
Функция ПроверитьФильтр(СтрокаТЖ, Фильтры)
	...
	Если Фильтры.Свойство("ТолькоСSQL") И Фильтры.ТолькоСSQL Тогда
			ЕстьСтрокаSql = СтрНайти(СтрокаТЖ, "Sql=");
			ЕстьСтрокаSdbl = СтрНайти(СтрокаТЖ, "Sdbl=");
			
			Если (ЕстьСтрокаSql + ЕстьСтрокаSdbl) = 0 Тогда
				Возврат Ложь;
			КонецЕсли;
	КонецЕсли;	
	
	Если Фильтры.Свойство("ТолькоСКонтекстом") И Фильтры.ТолькоСКонтекстом Тогда
			ЕстьСтрока = СтрНайти(СтрокаТЖ, "Context=");
			Если ЕстьСтрока = 0 Тогда
				Возврат Ложь;
			КонецЕсли;		
	КонецЕсли;
