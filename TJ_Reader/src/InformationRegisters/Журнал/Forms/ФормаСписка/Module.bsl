...
Процедура УстановитьОтбор()
	...
	Если СКонтекстом Тогда
		ЭлементОтбора = Список.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Заполнено;
		ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Context");
		ЭлементОтбора.Использование = Истина;		
	КонецЕсли;		
...
Процедура ПрочитатьТЖНаСервере()
	...
	Если Не ПустаяСтрока(usr) Или Не ПустаяСтрока(DataBase) Или ТолькоСSQL Или ТолькоСКонтекстом Тогда
		Фильтры = Новый Структура;
		Фильтры.Вставить("usr", usr);
		Фильтры.Вставить("DataBase", DataBase);
		Фильтры.Вставить("ТолькоСSQL", ТолькоСSQL);
		Фильтры.Вставить("ТолькоСКонтекстом", ТолькоСКонтекстом);
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Отметка");
